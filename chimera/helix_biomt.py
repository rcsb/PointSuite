# -----------------------------------------------------------------------------
# Chimera script to make image of single icos virus capsid from PDB with
# BIOMT records
#
...
def make_images(pdb_file):
     # open structure
     molecule = openModels.open(pdb_file, 'PDB')[0]

     #delete solvent
     runCommand('delete solvent')

     # Create multiscale surfaces
     d.make_multimers([molecule])
     ...
     #color 10 subunits by matrix index
     # 1 = yellow  2-5 = green, 6-10 = red
     # 11-20 = dark blue  21-40 = cyan  41-60 = white/grey
     #color_multiscale(yellow, 1, range(1,2))  
     #color_multiscale(green, 1, range(2,6))   
     #color_multiscale(red, 1, range(6,11)) 
     # color_multiscale(blue, 1, range(11,21))
     # color_multiscale(cyan, 1, range(21,41))
     # color_multiscale(grey, 1, range(41,61)) 

     # Capture and save image
     image_path = join(pdb_file + '.jpg')
